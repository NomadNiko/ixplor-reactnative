...
export const vendorsApi = {
  ...
  async getNearbyVendors(lat: number, lng: number, radius: number = 10000): Promise<VendorsResponse> {
    console.log('VendorsAPI - Fetching nearby vendors (client-side filtering):', { lat, lng, radius });
    ...
    const nearbyVendors = allVendors.data?.filter(vendor => {
  ...
  async getVendorsByType(type: 'tours' | 'lessons' | 'rentals' | 'tickets'): Promise<VendorsResponse> {
